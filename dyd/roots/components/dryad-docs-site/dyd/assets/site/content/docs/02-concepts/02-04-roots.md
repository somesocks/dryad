...
- `unpack=true` extracts a tar, tar.gz, tar.bz2, or zip archive before placement.
...
- `unpack=true` extracts a tar, tar.gz, tar.bz2, or zip archive. With `into=...`, the extracted archive is placed under a directory named after the archive.
- `fingerprint=<fingerprint>` pins the expected dependency package fingerprint.
