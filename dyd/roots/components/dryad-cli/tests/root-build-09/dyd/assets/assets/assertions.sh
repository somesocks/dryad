...
assert_directory_not_exists() {
    if [ -d "$1" ]; then
        echo "[ERROR] Fail: directory '$1' exists" 1>&2
        return 1
    fi
    # echo "[INFO] Pass: directory '$1' does not exist" 1>&2
}
...
assert_file_not_exists() {
    ...
    # echo "[INFO] Pass: file '$1' does not exist" 1>&2
}
