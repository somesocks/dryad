...
var rootRequirementAddCommand = func() clib.Command {
	...
	command := clib.NewCommand("add", "add a requirement to the current root").
		...
		WithOption(clib.NewOption("unpack", "for http requirements, unpack the downloaded tar, tar.gz, tar.bz2, or zip archive before placing it").WithType(clib.OptionTypeBool)).
