...
var rootDevelopStartCommand = func() clib.Command {
	...
	command := clib.NewCommand("start", "start a temporary development shell environment for a root").
		...
		WithOption(
			clib.NewOption(
				"variant",
				"select a root variant descriptor to develop (filesystem form: dimension=option+dimension=option). supports none/any/host; inherit is invalid. must resolve to exactly one concrete variant",
