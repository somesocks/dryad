...
func TestRootVariantsExclusionsLoad_Basic(t *testing.T) {
	...
	writeFileForTest(t, filepath.Join(rootPath, "dyd", "variants", "_exclude", "arch=arm64+os=darwin"), "false")
...
func TestRootVariantsExclusionsLoad_RejectsNonCanonicalDescriptor(t *testing.T) {
	...
	rootPath := t.TempDir()
	writeFileForTest(t, filepath.Join(rootPath, "dyd", "variants", "_exclude", "os=linux+arch=amd64"), "true")
